...
class Generator(torch.nn.Module):
    ...
    def forward(self, z, c, z2=None, truncation_psi=1, truncation_cutoff=None, update_emas=False, **synthesis_kwargs):
        ...
        ws_concat = torch.cat([ws, ws2], dim=-1) # [batch, num_layers, 2*w_dim]
        w_dim = ws.shape[-1]
        ws_combined = (ws_concat[:, :, ::2] + ws_concat[:, :, 1::2]) / 2  # [batch, num_layers, w_dim]

        #Normalize
        ws_combined = ws_combined / (ws_combined.norm(dim=-1, keepdim=True) + 1e-8)
